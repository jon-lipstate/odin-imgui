...
output_foreign :: proc(json_path: string, output_path: string, predefined_entites: []Predefined_Entity) {
    ...
    output_foreign_import :: proc(sb: ^strings.Builder) {
        fmt.sbprint(sb, "when ODIN_OS == .Windows {\n");
        ...
        fmt.sbprint(sb, "} else when ODIN_OS == .Linux {\n");
            fmt.sbprint(sb, "\tforeign import cimgui { \n");
                fmt.sbprint(sb, "\t\t\"system:stdc++\",\n");
                fmt.sbprint(sb, "\t\t\"external/cimgui.a\",\n");
            fmt.sbprint(sb, "\t}\n");
        fmt.sbprint(sb, "}\n");
    }
}
